...
namespace MFST {
	...
	bool Mfst::start() {
		...
		case LENTA_END: {
			...
			std::cout << std::setw(4) << std::left << 0 << ": всего строк " << lex.table[lex.size - 1].sn << " , синтаксический анализ выполнен без ошибок\n";
			rc = true;
			break;
		}
		case NS_NORULE: {
			MFST_TRACE4("------>NS_NORULE")
				std::cout << "------------------------------------------------------------------------------------------   ------\n";
			std::cout << getDiagnosis(0, buf) << '\n';
			std::cout << getDiagnosis(1, buf) << '\n';
			std::cout << getDiagnosis(2, buf) << '\n';
			break;
		}
